    ...
    try:
        while x <= maxlength:
            ...
            for score in jsonData["_leaderboardsData"][x]["_scores"]:
                ...
                if ((score['_playerName'] == 'CHRIS') or (score["_playerName"] == "PETER")):
                    filteredList.append(score)
                else:
                    if (score["_playerName"] == "SOPHIE"):
                        score["_playerName"] = "Harry in disguise"
                    if( ('American Idiot' in score['_leaderboardId']) or ('Welcome to the Family' in score['_leaderboardId'])
                        or ('Born To Be Yours' in score['_leaderboardId']) or ('Shut Up and Dance' in score['_leaderboardId'])
                        or ('Beat it' in score['_leaderboardId']) or ('Dance, Dance' in score['_leaderboardId'])
                        or ('Don\'t Stop Me Now' in score['_leaderboardId']) or ('This Is Gospel' in score['_leaderboardId'])
                        or('Waiting For Love' in score['_leaderboardId'])
                        or ('On Top of the World' in score['_leaderboardId']) or ('We Will Rock You' in score['_leaderboardId'])):
